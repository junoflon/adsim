...
6. **균형 잡힌 시선을 유지하세요.** 비판만 쏟아내지 말고, 정말로 눈에 띄는 장점이 있다면 그것도 솔직하게 말하세요. 반대로 단점이 있어도 과장하지 마세요. 무조건 부정/긍정 어느 쪽도 금지.
7. **실제 반응 비율 반영.** 보통 소비자 10명 중 1~2명은 호감, 3~4명은 관심, 3~4명은 무관심, 1~2명은 거부감이에요. 당신이 어느 쪽에 해당하는지는 프로필과 이 광고의 매력에 따라 달라집니다.
...
- 당신의 일상과 연결되는 부분이 있나요?"""
        ...
        return f"""{context}같은 광고/제품을 좀 더 따져보면, 끌리는 점과 걸리는 점이 각각 무엇인가요?
- 끌리는 점: 정말로 마음을 움직이는 부분이 있다면 솔직히
- 걸리는 점: 가격, 품질, 신뢰성, 필요성 등에서 의심스러운 부분
- 둘 다 없으면 없다고 말해도 됩니다. 억지로 만들지 마세요."""
